export default {
  ...
  methods: {
    ...
    emitDeselect(deselectedNodes, event) {
      this.getRoot().$emit("deselect", deselectedNodes, event);
    },
    ...
    select(path, addToSelection = false, event = null) {
      ...
      this.traverse((node, nodeModel) => {
        ...
        if (
          currentSelectedValue !== nodeModel.isSelected &&
          !nodeModel.isSelected &&
          nodeModel.isSelected !== undefined
        ) {
      ...
      if (deselectedNodes.length) this.emitDeselect(deselectedNodes, event);
      return selectedNode;
    },
    ...
    onNodeMouseupHandler(event, targetNode = null) {
      ...
      if (!this.isDragging && targetNode && !this.preventDrag) {
        this.select(targetNode.path, true, event);
      }
    ...
    copy(entity) {
      return Array.isArray(entity) ? [...entity] : Object.assign({}, entity);
    },
  },
};
